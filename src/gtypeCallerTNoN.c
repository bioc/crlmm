...
SEXP gtypeCallerPart1TNoN(SEXP A, SEXP B, SEXP fIndex, SEXP mIndex,
  ...
  */
  ...
  for (h=0; h < nSnpClasses; h++){
	...
	for (j=0; j < colsAB; j++){
	  ...
	  likelihood[j] = mydt( ((M[j]-f[j])-ptr2Centers[ii])/ptr2Scales[ii], ptr2df[0])*ptr2probs[0];

	  // likelihood for AB
	  likelihood[j+colsAB] = mydt( (M[j]-ptr2Centers[ii+rowsAB])/ptr2Scales[ii+rowsAB], ptr2df[0])*ptr2probs[1];
	  ...
	  // likelihood for BB
	  likelihood[j+2*colsAB] = mydt( ((M[j]+f[j])-ptr2Centers[ii+2*rowsAB])/ptr2Scales[ii+2*rowsAB], ptr2df[0])*ptr2probs[2];
...
SEXP gtypeCallerPart2TNoN(SEXP A, SEXP B, SEXP fIndex, SEXP mIndex,
		      SEXP theCenters, SEXP theScales, SEXP theNs,
		      SEXP Indexes, SEXP cIndexes, SEXP nIndexes,
		      SEXP ncIndexes, SEXP SMEDIAN,
		      SEXP knots, SEXP mixtureParams, SEXP df,
