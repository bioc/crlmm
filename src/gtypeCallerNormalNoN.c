...
static double mydn(double x, int df){
  return(exp(-pow(x, 2.0)/2.0));
}

SEXP gtypeCallerPart1NormalNoN(SEXP A, SEXP B, SEXP fIndex, SEXP mIndex,
  ...
  */
  ...
  for (h=0; h < nSnpClasses; h++){
	...
	for (j=0; j < colsAB; j++){
	  ...
	  likelihood[j] = mydn( ((M[j]-f[j])-ptr2Centers[ii])/ptr2Scales[ii], ptr2df[0])*ptr2probs[0];

	  // likelihood for AB
	  likelihood[j+colsAB] = mydn( (M[j]-ptr2Centers[ii+rowsAB])/ptr2Scales[ii+rowsAB], ptr2df[0])*ptr2probs[1];
	  ...
	  // likelihood for BB
	  likelihood[j+2*colsAB] = mydn( ((M[j]+f[j])-ptr2Centers[ii+2*rowsAB])/ptr2Scales[ii+2*rowsAB], ptr2df[0])*ptr2probs[2];
...
SEXP gtypeCallerPart2NormalNoN(SEXP A, SEXP B, SEXP fIndex, SEXP mIndex,
		      SEXP theCenters, SEXP theScales, SEXP theNs,
		      SEXP Indexes, SEXP cIndexes, SEXP nIndexes,
		      SEXP ncIndexes, SEXP SMEDIAN,
		      SEXP knots, SEXP mixtureParams, SEXP df,
